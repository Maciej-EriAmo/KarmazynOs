#!/usr/bin/env python3
"""
thermal_invaders.py — Thermal Invaders v2.10
=============================================
- Unikalne ID atomów dla każdej fali (brak ValueError)
- Łatwiejszy start (mniej wrogów, wolniejsze nagrzewanie)
- Działa w bąblu (zapis stanu przez VFS)
- Wyjście: F1
"""
...
PLAYER_SPEED = 380.0
BULLET_SIZE, BULLET_SPEED = 6, -600.0
ENEMY_WIDTH, ENEMY_HEIGHT = 28, 24
ENEMY_BASE_SPEED = 45.0
ENEMY_DROP_STEP = 16
PLAYER_SHOOT_DELAY = 0.25
ENEMY_SHOOT_DELAY = 1.4
LIVES = 5
SCORE_PER_KILL = 10
ENEMY_HEAT_RATE = 5.5
COOLING_PER_HIT = 28.0
...
class Enemy:
    def __init__(self, x, y, idx, runtime, wave_id=0, is_boss=False):
        self.x, self.y = x, y
        self.idx = idx
        ...
        if norm > 0: vec /= norm
        unique_id = f"invader_{wave_id}_{idx}"
        if HAS_KARMAZYN and runtime:
            if runtime.has_atom(unique_id):
                try:
                    ...
                    pass
            self.atom = runtime.create_atom(unique_id, vec, f"{x},{y}", 35.0)
        else:
            self.atom = Atom(unique_id, vec, f"{x},{y}", 35.0)
    ...
    def update_from_state(self):
        if self.state == "HOT":
            self.speed_x = ENEMY_BASE_SPEED * (2.2 if not self.is_boss else 1.6)
            self.shoot_cooldown = ENEMY_SHOOT_DELAY * (0.4 if not self.is_boss else 0.6)
        elif self.state == "WARM":
            self.speed_x = ENEMY_BASE_SPEED * 1.3
...
class ThermalInvaders:
    ...
    def _save_state(self):
        ...
        try:
            if hasattr(self.runtime, 'phi_store'):
                self.runtime.phi_store.save('thermal_invaders', json.dumps(state), subdir='games')
        except Exception:
            pass
    ...
    def _load_state(self):
        ...
        try:
            if hasattr(self.runtime, 'phi_store'):
                data = self.runtime.phi_store.load('thermal_invaders', subdir='games')
                if data:
                    state = json.loads(data)
                    self.level = state.get('level', 1)
                    self.score = state.get('score', 0)
                    self.lives = state.get('lives', LIVES)
                    self._wave_counter = state.get('wave_counter', 0)
        except Exception:
            pass
    ...
    def _cleanup_old_atoms(self):
        if not self.runtime:
            return
        for atom in self.runtime.matrix.atoms():
            if atom.id.startswith("invader_"):
                try:
                    self.runtime.delete_atom(atom.id)
                except:
                    pass

    def _create_enemies(self):
        self._cleanup_old_atoms()
        self._wave_counter += 1
        self.enemies.clear()
        is_boss_level = (self.level % 4 == 0)
        if is_boss_level:
            boss = Enemy(self.panel_w//2 - 45, 70, 0, self.runtime, self._wave_counter, is_boss=True)
            boss.speed_x = 38.0
            self.enemies.append(boss)
        else:
            cols = min(4, 2 + self.level // 2) if self.level <= 3 else min(6, 3 + self.level // 2)
            rows = min(2, 1 + self.level // 4) if self.level <= 2 else min(4, 2 + (self.level - 1) // 3)
            margin_x, margin_y = 45, 55
            spacing_x = ENEMY_WIDTH + 16
            spacing_y = ENEMY_HEIGHT + 14
            idx = 0
            for row in range(rows):
                for col in range(cols):
                    x = margin_x + col * spacing_x
                    y = margin_y + row * spacing_y
                    self.enemies.append(Enemy(x, y, idx, self.runtime, self._wave_counter))
                    idx += 1
    ...
    def _destroy_enemy(self, enemy: Enemy, give_points=True):
        ...
        if give_points:
            ...
            points = int(SCORE_PER_KILL * multiplier)
            if enemy.is_boss:
                points *= 10
            self.score += points
        self.explosions.append((enemy.x + (45 if enemy.is_boss else ENEMY_WIDTH//2),
                                ...
                                now + 0.4))
        if not enemy.is_boss and enemy.state == "HOT" and np.random.rand() < 0.25:
    ...
    def _handle_input(self, dt):
        ...
        now = time.monotonic()
        shoot_delay = 0.12 if now < self.rapid_fire_until else PLAYER_SHOOT_DELAY
        if keys[pygame.K_SPACE] and now - self.last_player_shot >= shoot_delay:
            ...
            if now < self.multi_shot_until:
                self.bullets.extend([(bx-16, by), (bx, by), (bx+16, by)])
            else:
    ...
    def _update_enemies(self, dt):
        ...
        if self.enemy_direction == 1 and max_x >= self.panel_w - 20:
            ...
            for e in self.enemies: e.y += ENEMY_DROP_STEP
        for e in self.enemies:
            if e.frozen_until > now: continue
            e.x += self.enemy_direction * e.speed_x * dt
            e.heat(ENEMY_HEAT_RATE * dt * (1.1 if self.level > 3 else 1.0))
            if not e.is_alive():
    ...
    def update(self, dt):
        if not self.active or self.game_over: return
    ...
    def draw(self, ctx):
        ...
        ctx.clear(C_BG)
        for e in self.enemies:
            color = C_BOSS if e.is_boss else C_ENEMY_HOT if e.state == "HOT" else C_ENEMY_WARM if e.state == "WARM" else C_ENEMY_COLD
            if e.frozen_until > now: color = C_FROZEN
            w = 90 if e.is_boss else ENEMY_WIDTH
            h = 50 if e.is_boss else ENEMY_HEIGHT
            pygame.draw.rect(ctx.surface, color, (e.x, e.y, w, h))
        pygame.draw.rect(ctx.surface, C_PLAYER, (self.player_x, self.player_y, PLAYER_WIDTH, PLAYER_HEIGHT))
        pygame.draw.rect(ctx.surface, (160, 240, 160), (self.player_x + PLAYER_WIDTH//2 - 4, self.player_y - 9, 8, 13))
        for bx, by in self.bullets:
...
def cmd_thermal(args, display, runtime=None):
    ...
    _THERMAL_GAME.start()
    return "Thermal Invaders v2.10 uruchomione! 🔥"

if __name__ == "__main__":
    print("Thermal Invaders v2.10 – uruchom przez: THERMAL")