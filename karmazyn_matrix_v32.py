...
Changes from v3.1:
  [1] Adaptive friction: friction = friction_margin / sqrt(dim)
      Eliminates hidden dimension-dependent hyperparameter.
      Geometric interpretation: threshold just above expected random similarity.

  [2] Momentum trace: Trace_t = normalize(α * weighted_sum + (1-α) * Trace_{t-1})
      Gives attractor inertia against transient noise spikes.
      Prevents trace hijacking when noise atoms start with high T.

  [3] Capacity analysis: capacity_test(n_noise, dim) sweeps noise atom count
      and measures core survival rate. Finds empirical capacity limit.

  [4] Multi-seed energy statistics: run_energy_stats(n_seeds)
      Reports E monotonicity across seeds — replaces single-seed anecdote.

  [5] Genesis 4 (Schrödinger's Cat): bind/unbind integrated into dynamics.
      "Kot goni mysz" encoded as HRR triple, injected as atom,
      trace unbinding used to recover relational components post-convergence.
      Cat enters the test. Cat may or may not survive. We measure both outcomes.
"""
...
class KarmazynMatrix:
    ...
    def energy(self) -> float:
        """E = -sum_i T_i * <S_i, Trace>. Lower = more coherent."""
        if not self.atoms:
            return 0.0
...
if __name__ == "__main__":
    print("KarmazynMatrix v3.2 — Full Test Suite")
    print("Adaptive friction | Momentum trace | Capacity | Multi-seed | Genesis 4")
    ...
    # Find capacity threshold (first n_noise where survival < 80%)
    threshold = next((r["n_noise"] for r in cap if r["survival_rate"] < 0.8), None)
    print(f"\n  Empirical capacity threshold: ~{threshold} noise atoms "
          f"(survival drops below 80%)")
    ...
    print(f"  Inverted-T core survival:     alpha=0.3 → "
          f"{inv[0.3]['core_survived']}/3 core atoms")
    print(f"  Capacity threshold:           ~{threshold} noise atoms")
    print(f"  Energy clean post-20:         {stats['pct_clean_post20']:.0%} of 50 seeds")
    print(f"  Genesis 4 external cat:       {'survived' if cat['external_cat_survived'] else 'collapsed (expected)'}")
    print(f"  Genesis 4 internal cat:       {'survived' if cat['internal_cat_survived'] else 'collapsed'}")
    print(f"  Relational recovery (trace):  {cat['relational_recovery_from_trace']:.4f}")
