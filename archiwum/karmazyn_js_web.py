"""
karmazyn_js_web.py — JS Web Bridge dla Lunety v1.2
====================================================
...
Zmiany v1.2:
  - USUNIĘTO destruktywny cache plików JS w atomach (ucięcia kodu do 8KB powodowały EOF error w parserze).
  - Skrypty zewnętrzne używają `http_get`, więc kompresja (gzip/brotli) od CDN jest poprawnie dekodowana.
"""
...
class JSBridge:
    ...
    def _run_external(self, url: str) -> Tuple[bool, str]:
        ...
        try:
            # Właściwe pobieranie z dekompresją (gzip/brotli) przez mechanizm Lunety
            resp = http_get(full_url)
            if not resp or not resp.ok():
                return False, f"fetch failed: status {resp.status if resp else 0}"
            code = resp.text
        ...
        # Przekazujemy CZYSTY, pełny kod do wykonania. Zlikwidowano destruktywny cache w Phi-Space.
