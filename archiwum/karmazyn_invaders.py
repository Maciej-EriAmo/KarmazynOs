#!/usr/bin/env python3
"""
thermal_invaders.py — Thermal Invaders v2.0 (fully KarmazynOS-compliant)
========================================================================
- Wszystkie byty używają wektorów phi-space (15 wymiarów).
- Gra nie ingeruje w runtime poza tworzeniem atomów i schedulerem.
- Scheduler działa synchronicznie (co klatkę, a nie w tle).
- Termodynamika: wrogowie nagrzewają się z czasem, przechodząc COLD→WARM→HOT.
- Rezonans pocisku z wrogiem (cos_sim >= 0.65) zadaje obrażenia (chłodzenie).
- Żywotność wroga kończy się, gdy T >= 100 (przegrzanie i rozpad) lub T <= 0 (wygaszenie).
"""
...
# Importujemy tylko to, co potrzebne z KarmazynOS
try:
    ...
    HAS_KARMAZYN = False
    # Fallbackowe definicje dla trybu demo
    T_HOT, T_WARM, T_COLD, T_TOMB = 70.0, 30.0, 10.0, 2.0
    class Atom:
        def __init__(self, atom_id, S, E, T):
            self.id = atom_id
...
# ========== Stałe gry ==========
PHI_DIM = 15                     # wymiar przestrzeni phi (zgodny z KarmazynOS)

PLAYER_WIDTH     = 30
PLAYER_HEIGHT    = 20
PLAYER_SPEED     = 400.0          # px/s
BULLET_SIZE      = 6
...
# Termodynamika – zgodna z karmazyn_atom.py
ENEMY_HEAT_RATE   = 8.0            # punkty T na sekundę
RESONANCE_THRESH  = 0.65           # próg cos_sim dla trafienia
COOLING_PER_HIT   = 25.0           # ile T zabiera trafienie
...
# ========== Klasa wroga (opakowanie atomu) ==========
class Enemy:
    def __init__(self, x: float, y: float, idx: int, runtime):
        self.x = x
        self.y = y
        self.idx = idx
        self.atom = None
        ...
        # Generujemy wektor semantyczny – pozycja (x,y) + losowy szum
        # Ważne: wymiar PHI_DIM, wartości znormalizowane do sfery jednostkowej
        vec = np.random.randn(PHI_DIM).astype(np.float32)
        # Dodajemy sygnał z pozycji – w praktyce kodujemy x,y w pierwszych dwóch współrzędnych
        vec[0] = x / 100.0
        vec[1] = y / 100.0
        ...
        if HAS_KARMAZYN and runtime:
            self.atom = runtime.create_atom(f"invader_{idx}", vec, f"{x},{y}", 40.0)
        else:
            self.atom = Atom(f"invader_{idx}", vec, f"{x},{y}", 40.0)
    ...
    def is_alive(self) -> bool:
        """Żywy = nie jest TOMB i T w przedziale (0, 100)."""
        if not self.atom:
            return False
        return self.atom.state != "TOMB" and 0 < self.atom.T < 100

    def update_from_state(self):
        """Dostosowuje szybkość i tempo strzałów do stanu termicznego."""
        if not self.atom:
            return
...
class ThermalInvaders:
    def __init__(self, display, runtime=None):
        ...
        self.bullets: List[Tuple[float, float, np.ndarray]] = []   # (x, y, wektor)
        self.enemy_bullets: List[Tuple[float, float]] = []
        self.explosions: List[Tuple[float, float, float]] = []      # (x, y, end_time)

        self.last_player_shot = 0.0
        self.last_enemy_shot = 0.0
        self.enemy_direction = 1
        self._last_heat_time = 0.0   # do synchronizacji nagrzewania

        # Rejestracja eventów (tylko jeśli runtime)
        if HAS_KARMAZYN and runtime:
            runtime.events.on("state_changed", self._on_enemy_state_changed)

    def _on_enemy_state_changed(self, atom):
        for enemy in self.enemies:
            if enemy.atom is atom:
                enemy.update_from_state()
                break

    def start(self):
        if not self.display or not self.display.available:
            print("Tryb graficzny niedostępny.")
            return
        if self.renderer is None:
            print("Renderer niedostępny.")
            return
        self.active = True
        self._initialized = False
    ...
    def _reset_game(self):
        ...
        self.enemy_direction = 1
        self.last_player_shot = time.monotonic()
        self.last_enemy_shot = time.monotonic()
        self._create_enemies()
        self._last_heat_time = time.monotonic()
    ...
    def _create_enemies(self):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                enemy = Enemy(x, y, idx, self.runtime)
    ...
    def _destroy_enemy(self, enemy: Enemy, give_points=True):
        ...
        self.explosions.append((enemy.x + ENEMY_WIDTH//2, enemy.y + ENEMY_HEIGHT//2,
                                time.monotonic() + 0.3))
        # Atom pozostaje w phi-space – stygnie naturalnie (może być reaktywowany?)
        # Nie usuwamy go – runtime sam oczyści TOMB.
    ...
    def _fire_bullet(self):
        # Wektor pocisku = kierunek w górę (0, -1) w przestrzeni phi
        # Używamy pełnego wymiaru PHI_DIM – tylko składowe y ujemne
        vec = np.zeros(PHI_DIM, dtype=np.float32)
        vec[1] = -1.0   # składowa "y"
        norm = np.linalg.norm(vec)
        if norm > 0:
            vec /= norm
        bx = self.player_x + PLAYER_WIDTH//2 - BULLET_SIZE//2
        by = self.player_y
        self.bullets.append((bx, by, vec))

    def _update_bullets(self, dt):
        # Pociski gracza
        new_bullets = []
        for bx, by, vec in self.bullets:
            by += BULLET_SPEED * dt
            if by + BULLET_SIZE > 0:
                new_bullets.append((bx, by, vec))
                for enemy in self.enemies[:]:
                    if (bx + BULLET_SIZE > enemy.x and bx < enemy.x + ENEMY_WIDTH and
                        by + BULLET_SIZE > enemy.y and by < enemy.y + ENEMY_HEIGHT):
                        # Rezonans – iloczyn skalarny wektorów phi
                        sim = float(np.dot(vec, enemy.atom.S))
                        if sim >= RESONANCE_THRESH:
                            enemy.cool(COOLING_PER_HIT * sim)
                            if not enemy.is_alive():
                                self._destroy_enemy(enemy, give_points=True)
                            break  # pocisk znika po trafieniu
    ...
    def _update_enemies(self, dt):
        ...
        if self.enemies and now - self.last_enemy_shot >= ENEMY_SHOOT_DELAY:
            ...
            # Wybór wroga z najwyższą temperaturą
        ...
        # Termodynamika – nagrzewanie (synchroniczne z dt)
        # Aby nie nagrzewać co klatkę zbyt szybko, używamy akumulatora czasu
        # Ale dla uproszczenia nagrzewamy co klatkę, skalowane dt.
        # Lepiej: nagrzewaj co 0.2 sekundy, niezależnie od FPS.
        # Użyjemy prostego rozwiązania: heat_rate * dt
        for e in self.enemies:
            e.heat(ENEMY_HEAT_RATE * dt)
            if not e.is_alive():
                self._destroy_enemy(e, give_points=False)   # przegrzanie – brak punktów

        # Dodatkowo – dotarcie wrogów do dołu = przegrana
        for e in self.enemies:
    ...
    def draw(self, ctx):
        ...
        # Wrogowie – kolor w zależności od stanu termicznego
        for e in self.enemies:
            if e.state == "HOT":
                color = C_ENEMY_HOT
            elif e.state == "WARM":
        ...
        # UI
        font = ctx.font
        score_txt = font.render(f"SCORE: {self.score}", True, C_UI)
        lives_txt = font.render(f"LIVES: {self.lives}", True, C_UI)
        ctx.surface.blit(score_txt, (ctx.rect.x + 10, ctx.rect.y + 10))
        ctx.surface.blit(lives_txt, (ctx.rect.x + 10, ctx.rect.y + 40))

        if self.game_over:
            msg = "YOU WIN!  R = restart, ESC = quit" if self.won else "GAME OVER!  R = restart, ESC = quit"
...
# ========== Globalna instancja i komenda shella ==========
_THERMAL_GAME: Optional[ThermalInvaders] = None

def cmd_thermal(args, display, runtime=None):
    global _THERMAL_GAME
    if not display or not display.available:
        return "Tryb graficzny niedostępny. Uruchom DISPLAY."
    if _THERMAL_GAME is not None and _THERMAL_GAME.active:
        return "Gra już działa. Użyj ESC, aby wyjść."
    _THERMAL_GAME = ThermalInvaders(display, runtime)
    _THERMAL_GAME.start()
    return "Thermal Invaders v2.0 – strzałki, spacja, ESC."

if __name__ == "__main__":
    print("Thermal Invaders – uruchom przez shell KarmazynOS: THERMAL")