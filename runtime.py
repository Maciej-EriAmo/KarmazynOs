...
Zmiany v1.3:
    [fix] matrix.atoms() — metoda, nie atrybut
    [fix] matrix.step()  — iterator (atom, event_type)
    [fix] Atom to obiekt, nie dict
    [new] PhiSpace, Bubble, Hologram, Agent — semantyczne warstwy .karm
    [new] write(), consolidate(), recall(), archive_to_hologram()
    [new] generate_from_idea(), refresh_bubble(), revoke_bubble()
    [new] mark_bubble_for_decay(), atom_id_for(), derive_agent()
    [new] read_as_agent()
"""
...
class SanctuaryRuntime:
    ...
    def corrupt_atom(self, id: str, amount: float = 25):
        ...
        if not atom:
            raise ValueError(f"Atom '{id}' nie istnieje")
        # amount może być w skali 0..100 lub 0..1
    ...
    def start_loop(self, interval: float = 0.2):
        ...
        threading.Thread(target=loop, daemon=True, name="runtime-loop").start()

    def stop_loop(self):
        self._running = False

    def start_system_loop(self, interval: float = 0.2):
        """Alias dla kompatybilności z hss_demo.py."""
    ...
    def start_mission(self, mission: dict):
        ...
        # startowa_zywica z misji nadpisuje domyślne 10
        ...
        for r in mission.get("relikwie", []):
            try:
                self.create_atom(
                    str(r["id"]),
                    str(r.get("S", "")),
                    str(r.get("E", "")),          # np.str_ → str
                    float(r.get("T_start",        # LevelEmbedder używa T_start
                           r.get("T", 80.0))),
                )
            except ValueError:
                pass
    ...
    def write(self, name: str, S: str, E: str, T: float) -> str:
        """
        Tworzy atom dla języka .karm.
        T ∈ [0,1] — skala .karm, konwertowana do 0..100 w matrix.
        Rejestruje name → atom.id w _name_to_id.
        Zwraca atom.id.
        """
        # Unikalny id: name, name_1, name_2 ...
        label  = name
        suffix = 0
        ...
        # T z .karm jest w 0..1 → konwertujemy do 0..100
    ...
    def consolidate(self, label: str, metadata: dict = None) -> str:
        """
        Tworzy bąbel z istniejącego atomu.
        Emituje atom_stabilized → audio.mandala_harmony().
        """
    ...
    def recall(self, query: str, k: int = 5) -> List[dict]:
        """Przeszukuje PhiSpace + aktywne bąble semantycznie."""
    ...
    def archive_to_hologram(self, topic: str, atom_ids: List[str],
        ...
        """Tworzy hologram PCA z wektorów semantycznych atomów."""
        vectors   = []
        valid_ids = []
        for lbl in atom_ids:
            v = self.phi.get(lbl)
    ...
    def generate_from_idea(self, hologram_id: str, prompt: str,
        ...
        """Generuje wektor semantyczny z hologramu przez projekcję."""
    ...
    def refresh_bubble(self, label: str) -> bool:
        """Stabilizuje atom → liveliness bąbla rośnie do 1.0."""
    ...
    def revoke_bubble(self, label: str) -> bool:
        """Warp Oblivion — atom → TOMB, emituje vacuum_decay."""
    ...
    def mark_bubble_for_decay(self, label: str, rate: float = 0.01) -> bool:
        """Inicjuje zanik — corrupt proporcjonalny do rate * T_max."""
        atom = self.matrix.get_atom(label)
        if atom is None:
            return False
